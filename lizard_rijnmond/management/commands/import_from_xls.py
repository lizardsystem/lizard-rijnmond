...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Preprocess relative_level.
        for reference_result in RiverlineResult.objects.filter(is_reference=True):
            logger.info("Preprocessing results relative to %s", reference_result)
            reference_data = {}
            for data in RiverlineResultData.objects.filter(
                riverline_result=reference_result):
                reference_data[data.location] = data.level
            for result in RiverlineResult.objects.filter(
                is_reference=False,
                scenario=reference_result.scenario,
                year=reference_result.year):
                logger.debug("Preprocessing results %s", result)
                for data in RiverlineResultData.objects.filter(
                    riverline_result=result):
                    data.relative_level = (
                        data.level - reference_data[data.location]) * 100
                    data.save()
